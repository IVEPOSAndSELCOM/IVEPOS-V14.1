...
public class PL2303Driver extends USBDriver {
    ...
    int attach(USBPort port) {
        ...
        pl2303_vendor_read(port, 0x8484, 0, buf);
        pl2303_vendor_write(port, 0x0404, 0);
        pl2303_vendor_read(port, 0x8484, 0, buf);
        pl2303_vendor_read(port, 0x8383, 0, buf);
        pl2303_vendor_read(port, 0x8484, 0, buf);
        ...
        if (type == pl2303_type.HX)
            ...
            pl2303_vendor_write(port, 2, 0x24);
    ...
    int open(USBPort port, TTYTermios serial) {
        ...
        if (type != pl2303_type.HX) {
            ...
            pl2303_vendor_write(port, 8, 0);
            pl2303_vendor_write(port, 9, 0);
        }
        set_termios(port, serial);
    ...
    /**
     * ???termiosnew????????ô????termiosnew????????2303 ?????serial???termios???termiosnew
     */
    private int set_termios(USBPort port, TTYTermios termiosnew) {
        ...
        if (termiosnew.flowControl == TTYTermios.FlowControl.DTR_RTS) {
            if (type == pl2303_type.HX)
    ...
    public static class TTYTermios implements Parcelable {
        /**
         * ???????????9600,19200,38400,57600,115200 ?????SDK????û????????????????????????
         */
