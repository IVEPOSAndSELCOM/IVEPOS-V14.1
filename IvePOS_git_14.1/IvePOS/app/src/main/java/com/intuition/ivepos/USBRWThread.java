...
public class USBRWThread extends Thread {
    ...
    private static final Object SLOCK = new Object(); // ????????????????á?
    ...
    public static boolean Open(USBDriver.USBPort port, PL2303Driver.TTYTermios serial) {
    ...
    private static boolean _Open(USBDriver.USBPort port, PL2303Driver.TTYTermios serial) {
        boolean valid = false;

        try {
            if (pl2303.probe(port, PL2303Driver.id) == USBDriver.RTNCode.OK) {
                if (pl2303.attach(port) == USBDriver.RTNCode.OK) {
                    if (pl2303.open(port, serial) == USBDriver.RTNCode.OK) {
