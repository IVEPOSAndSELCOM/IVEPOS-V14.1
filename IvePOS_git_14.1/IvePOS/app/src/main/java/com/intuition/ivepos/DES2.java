...
public class DES2 {
    ...
    byte[][] szSubKeys = new byte[16][48];// ????16??48????
    byte[] szCiphertextRaw = new byte[64]; // ?????????????(64??Bits) int 0,1
    byte[] szPlaintextRaw = new byte[64]; // ?????????????(64??Bits) int 0,1
    byte[] szCiphertextInBytes = new byte[8];// ????8?????
    byte[] szPlaintextInBytes = new byte[8];// ????8??????????

    byte[] szCiphertextInBinary = new byte[65]; // ?????????????(64??Bits) U8
    // '0','1',???????'\0'
    byte[] szCiphertextInHex = new byte[17]; // ???????????????,???????'\0'
    byte[] szPlaintext = new byte[9];// ????8??????????,???????'\0'
    ...
    // ????:???????????????
    // ????:???????????,????
    // ???:??????????????????private szFCiphertextAnyLength[8192]
    // ?û????????CiphertextAnyLength?õ?
    ...
    // ????:??????????????????????
    // ????:???????????,????
    // ???:??????????????????private szFPlaintextAnyLength[8192]
    // ?û????????PlaintextAnyLength?õ?
